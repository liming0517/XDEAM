...
public class Svl_ClassSet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ClassSetBean bean = new ClassSetBean(request);
		...
		if("query".equalsIgnoreCase(active)){
			...
			try {
				jsonV = bean.queryRec(pageNum, pageSize, BJBH_CX, BJMC_CX, XBDM_CX, NJDM_CX, SSZY_CX);
				jal = (JSONArray)jsonV.get(2);
				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
		...
		//查询班级教室下拉框
		if("loadClassroomCombo".equalsIgnoreCase(active)){
			try {
				jsonV = bean.loadClassroomCombo();
				jal = (JSONArray) jsonV.get(2);
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
				e.printStackTrace();
		...
		if("openTeacher".equalsIgnoreCase(active)){
			...
			try {
				jsonV=bean.openTeacher(pageNum, pageSize, teaId, teaName, teacharr);
				jal = (JSONArray)jsonV.get(2);
    ...
    * @param MajorSetBean
    */
	private void getFormData(HttpServletRequest request, ClassSetBean bean){
		...
		bean.setJSBH(MyTools.StrFiltr(request.getParameter("BJJS"))); //教室编号
	}
}