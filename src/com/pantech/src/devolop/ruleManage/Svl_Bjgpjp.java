...
public class Svl_Bjgpjp extends HttpServlet {
	/**
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BjgpjpBean bean = new BjgpjpBean(request);
		...
		if("queryTree".equalsIgnoreCase(active)){
			...
			try {
				//查询列表
				jsonV = bean.queryTree(pageNum, pageSize, parentId);
				jal = (JSONArray)jsonV.get(2);
		...
		//bjgpjpList_bz.jsp查询(不用)
		if ("loadJCSJ".equalsIgnoreCase(active)) {
			try {
				...
				//查询
				String times = bean.loadGPJP2();
				jal = JsonUtil.addJsonParams(jal, "gpjpData", times);
				
				response.getWriter().write(jal.toString());
				TraceLog.Trace("gpjp-----------------------"+jal.toString());
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
		...
		if ("checkTeaCls".equalsIgnoreCase(active)) {
			...
			try {
				bean.checkTeaCls(jsxm,skzcxq,weeks,cdyq,iKeyCode,termid,classId,jsbh);
		...
		if ("checkAddAll".equalsIgnoreCase(active)) {
			String idVal = MyTools.StrFiltr(request.getParameter("idVal"));
			try {
				jsonV = bean.checkAddAll(idVal);
				jal = (JSONArray)jsonV.get(2);
				jal = JsonUtil.addJsonParams(jal, "msg", bean.getMSG());
				response.getWriter().write(jal.toString());
				TraceLog.Trace(jal.toString());
		...
		if ("checkdeljin".equalsIgnoreCase(active)) {
			String idVal = MyTools.StrFiltr(request.getParameter("idVal"));
			try {
				bean.checkdeljin(idVal);
				jal = JsonUtil.addJsonParams(jal, "msg", bean.getMSG());
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
				e.printStackTrace();
    ...
    * @param XnxqBean
    */
	private void getFormData(HttpServletRequest request,BjgpjpBean bean){
