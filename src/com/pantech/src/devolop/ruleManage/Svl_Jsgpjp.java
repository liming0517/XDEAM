...
public class Svl_Jsgpjp extends HttpServlet {
	/**
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		JsgpjpBean bean = new JsgpjpBean(request);
...
//				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
				...
				TraceLog.Trace("list-------------------------"+jal.toString());
		...
		if("queryTree".equalsIgnoreCase(active)){
			...
			try {
				//查询列表
				jsonV = bean.queryTree(pageNum, pageSize,teaId,teaName);
				jal = (JSONArray)jsonV.get(2);
				response.getWriter().write("{\"total\":" + MyTools.StrFiltr(jsonV.get(0)) + ",\"rows\":" + jal.toString() + "}");

				TraceLog.Trace("list2-------------------------"+jal.toString());
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
...
//				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
		...
		if("initGridData".equalsIgnoreCase(active)){
			try {
				...
				jal = (JSONArray)jsonV.get(2);
				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
				//TraceLog.Trace("listData:---------"+"{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
				//查询节次时间
				jsonV = bean.loadJCSJ();
		...
		if ("checkTeaCls".equalsIgnoreCase(active)) {
			...
			String termid = MyTools.StrFiltr(request.getParameter("termid"));
			String teacherId = MyTools.StrFiltr(request.getParameter("teacherId"));
			String classdm = MyTools.StrFiltr(request.getParameter("classdm"));
			String jsbh = MyTools.StrFiltr(request.getParameter("jsbh"));
			try {
				bean.checkTeaCls(jsxm,skzcxq,weeks,cdyq,iKeyCode,termid,teacherId,classdm,jsbh);
				jal = JsonUtil.addJsonParams(jal, "msg", bean.getMSG());
		...
		if("loadjinpai".equalsIgnoreCase(active)){
			try {
				jsonV = bean.loadjinpai();
				jal = JsonUtil.addJsonParams(jal, "loadjinpai", ((JSONArray)jsonV.get(2)).toString());
				
				jsonV = bean.loadallsjxl();
				jal = JsonUtil.addJsonParams(jal, "loadallsjxl", ((JSONArray)jsonV.get(2)).toString());
				
				response.getWriter().write(jal.toString());
				TraceLog.Trace("loadjinpai:---------"+jal.toString());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
    ...
    * @param XnxqBean
    */
	private void getFormData(HttpServletRequest request,JsgpjpBean bean){
		...
		bean.setiUSERCODE(MyTools.StrFiltr(request.getParameter("iUSERCODE"))); //权限
	}
}
