...
public class HkszBean {
	 ...
	 * @param request
	 */
	public HkszBean(HttpServletRequest request) {
	 ...
	 * @throws SQLException
	 */
	public Vector loadStuHkmd(String ic_cjlx)throws SQLException{
		Vector vec = null;
		...
		if("ps".equalsIgnoreCase(ic_cjlx)){
			sql = "select 学号,姓名 from V_成绩管理_学生成绩信息表 where 相关编号='" + MyTools.fixSql(this.getCX_XGBH()) + "' and 成绩状态='1' and 平时='-5'";
		}
		if("qz".equalsIgnoreCase(ic_cjlx)){
			sql = "select 学号,姓名 from V_成绩管理_学生成绩信息表 where 相关编号='" + MyTools.fixSql(this.getCX_XGBH()) + "' and 成绩状态='1' and 期中='-5'";
		}
		if("sx".equalsIgnoreCase(ic_cjlx)){
			sql = "select 学号,姓名 from V_成绩管理_学生成绩信息表 where 相关编号='" + MyTools.fixSql(this.getCX_XGBH()) + "' and 成绩状态='1' and 实训='-5'";
		}
		if("qm".equalsIgnoreCase(ic_cjlx)){
			sql = "select 学号,姓名 from V_成绩管理_学生成绩信息表 where 相关编号='" + MyTools.fixSql(this.getCX_XGBH()) + "' and 成绩状态='1' and 期末='-5'";
		}
		vec = db.GetContextVector(sql);
	 ...
	 * @throws SQLException
	 */
	public void saveHkStu(String ic_cjlx)throws SQLException{
		Vector sqlVec = new Vector();
		String sql = ""; 
		
		if("ps".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 平时 = (case 平时 when '-5' then '' else 平时 end) where 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("qz".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期中 =(case 期中 when '-5' then '' else 期中 end) where 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("sx".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 实训 =(case 实训 when '-5' then '' else 实训 end) where 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("qm".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期末 =(case 期末 when '-5' then '' else 期末 end) where 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		sqlVec.add(sql);
		
		if("ps".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 平时 = '-5' where 学号 in ('" + this.getCX_XH().replaceAll(",", "','") + "') AND 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("qz".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期中 = '-5' where 学号 in ('" + this.getCX_XH().replaceAll(",", "','") + "') AND 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("sx".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 实训 = '-5' where 学号 in ('" + this.getCX_XH().replaceAll(",", "','") + "') AND 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		if("qm".equalsIgnoreCase(ic_cjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期末 = '-5' where 学号 in ('" + this.getCX_XH().replaceAll(",", "','") + "') AND 相关编号 = '" + MyTools.fixSql(this.getCX_XGBH()) +"'";
		}
		sqlVec.add(sql);
	...
	public Vector loadStuCourseListData(int pageNum, int pageSize) throws SQLException {
		...
		sql = "select distinct a.编号,a.学号,d.班内学号,a.姓名,b.课程名称,b.来源类型," +
			"case b.来源类型 when '2' then '添加课程' when '3' then '选修课程' when '4' then '分层课程' else '普通课程' end as 课程类型," +
			"case when a.期末='-5' or a.总评='-5' then 'hk' else '' end as 成绩状态 " +
			"from V_成绩管理_学生成绩信息表 a " +
			"left join V_成绩管理_登分教师信息表 b on b.相关编号=a.相关编号 " +
			"left join V_成绩管理_科目课程信息表 c on c.科目编号=b.科目编号 " +
			"left join V_学生基本数据子类 d on d.学号=a.学号 " +
			"where a.状态='1' and b.状态='1' and c.状态='1' " +
			"and a.成绩状态='1' and d.学生状态 in ('01','05')";
		...
		sql += " order by (case when d.班内学号='' then '9999' else d.班内学号 end),b.来源类型,b.课程名称";
		vec = db.getConttexJONSArr(sql, pageNum, pageSize);// 带分页返回数据(json格式）
		return vec;
	}
	 ...
	 * @throws SQLException
	 */
	public void saveStuHkCourse(String ic_plcjlx)throws SQLException{
		Vector sqlVec = new Vector();
		String sql = ""; 
		
		if("ps".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 平时 = (case 平时 when '-5' then '' else 平时 end) " +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("qz".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期中 =(case 期中 when '-5' then '' else 期中 end) " +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("sx".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 实训 =(case 实训 when '-5' then '' else 实训 end) " +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("qm".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期末 =(case 期末 when '-5' then '' else 期末 end) " +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("ps".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 平时 = '-5'" +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
				...
				if(!"请输入学生完整姓名".equalsIgnoreCase(this.getCX_XM()))
					sql += " and a.姓名='" + MyTools.fixSql(this.getCX_XM()) + "'";
				sql += " and 编号  in ('" + this.getCX_XGBH().replaceAll(",", "','") + "'))";
		}
		if("qz".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期中 = '-5'" +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("sx".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 实训 = '-5'" +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
		...
		if("qm".equalsIgnoreCase(ic_plcjlx)){
			sql = "update V_成绩管理_学生成绩信息表 set 期末 = '-5'" +
					"where 编号 in (select a.编号 from V_成绩管理_学生成绩信息表 a " +
					"left join V_成绩管理_科目课程信息表 b on b.科目编号=a.科目编号 " +
					"left join V_学生基本数据子类 c on c.学号=a.学号 " +
					"where a.状态='1' and b.状态='1' and a.成绩状态='1' and c.学生状态 in ('01','05') " +
	 ...
	 * @throws SQLException
	 */
	public Vector loadStuPLHkmd(String ic_plcjlx)throws SQLException{
		String sql = ""; // 查询用SQL语句
		Vector vec = null; // 结果集
		Vector resultVec = new Vector();
		String stuCode = "";
