...
public class Svl_Hksz extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		HkszBean bean = new HkszBean(request);
		...
		//读取所选课程学生缓考名单数据
		if("loadStuHkmd".equalsIgnoreCase(active)){
			try {
				//获取表单里缓考成绩类型
				String ic_cjlx = MyTools.StrFiltr(request.getParameter("ic_cjlx"));
				jsonV = bean.loadStuHkmd(ic_cjlx);
		...
		//保存缓考学生
		if("saveHkStu".equalsIgnoreCase(active)){
			try {
				//获取表单里缓考成绩类型
				String ic_cjlx = MyTools.StrFiltr(request.getParameter("ic_cjlx"));
				bean.saveHkStu(ic_cjlx);
		...
		//保存指定学生缓考课程信息
		if("saveHkPlsz".equalsIgnoreCase(active)){
			try {
				//获取表单里批量缓考成绩类型
				String ic_plcjlx = MyTools.StrFiltr(request.getParameter("plcjlx"));
				bean.saveStuHkCourse(ic_plcjlx);
				jal = JsonUtil.addJsonParams(jal, "MSG", bean.getMSG());
				response.getWriter().write(jal.toString());
			} catch (Exception e) {
				// TODO: handle exception
		...
		//根据成绩类型批量查询指定学生缓考编号信息
		if("loadStuPLHkmd".equalsIgnoreCase(active)){
			try {
				//获取批量设置里表单的查询条件
				bean.setCX_XH(URLDecoder.decode(bean.getCX_XH(), "UTF-8"));
				bean.setCX_XM(URLDecoder.decode(bean.getCX_XM(), "UTF-8"));
				//获取表单里批量缓考成绩类型
				String ic_plcjlx = MyTools.StrFiltr(request.getParameter("ic_plcjlx"));
				jsonV = bean.loadStuPLHkmd(ic_plcjlx);
				jal = JsonUtil.addJsonParams(jal, "stuCode", MyTools.StrFiltr(jsonV.get(0)));
				response.getWriter().write(jal.toString());
			} catch (Exception e) {
				// TODO: handle exception
    ...
    * @param HkszBean
    */
	private void getFormData(HttpServletRequest request,HkszBean bean){
