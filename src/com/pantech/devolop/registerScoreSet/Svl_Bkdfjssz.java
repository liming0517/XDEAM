...
public class Svl_Bkdfjssz extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BkdfjsszBean bean = new BkdfjsszBean(request);
		this.getFormData(request, bean); //获取SUBMIT提交时的参数（AJAX适用）
		
		//初始化补考课程登分信息
		if("initBkCourseInfo".equalsIgnoreCase(active)){
			try {
				bean.initBkCourseInfo();
		...
		if("queCourseList".equalsIgnoreCase(active)){
			...
			try {
				jsonV = bean.queCourseList(pageNum, pageSize, XN_CX, XBDM_CX, ZYDM_CX, NJDM_CX, KCMC_CX, KCLX_CX);
    ...
    * @param DfryszBean
    */
	private void getFormData(HttpServletRequest request,BkdfjsszBean bean){
