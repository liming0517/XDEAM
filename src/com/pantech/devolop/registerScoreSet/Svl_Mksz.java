...
public class Svl_Mksz extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		MkszBean bean = new MkszBean(request);
		...
		//读取所选课程学生免考名单数据
		if("loadStuMkmd".equalsIgnoreCase(active)){
			try {
				//获取表单里免考成绩类型
				String ic_cjlx = MyTools.StrFiltr(request.getParameter("ic_cjlx"));
				jsonV = bean.loadStuMkmd(ic_cjlx);
		...
		//保存免考学生
		if("saveMkStu".equalsIgnoreCase(active)){
			try {
				//获取表单里免考成绩类型
				String ic_cjlx = MyTools.StrFiltr(request.getParameter("ic_cjlx"));
				bean.saveMkStu(ic_cjlx);
		...
		//保存指定学生免考课程信息
		if("saveMkPlsz".equalsIgnoreCase(active)){
			try {
				//获取表单里批量免考成绩类型
				String ic_plcjlx = MyTools.StrFiltr(request.getParameter("plcjlx"));
				bean.saveStuMkCourse(ic_plcjlx);
				jal = JsonUtil.addJsonParams(jal, "MSG", bean.getMSG());
				response.getWriter().write(jal.toString());
			} catch (Exception e) {
				// TODO: handle exception
		...
		//根据成绩类型批量查询指定学生免考编号信息
		if("loadStuPLMkmd".equalsIgnoreCase(active)){
			try {
				//获取批量设置里表单的查询条件
				bean.setCX_XH(URLDecoder.decode(bean.getCX_XH(), "UTF-8"));
				bean.setCX_XM(URLDecoder.decode(bean.getCX_XM(), "UTF-8"));
				...
				jsonV = bean.loadStuPLMkmd(ic_plcjlx);
				jal = JsonUtil.addJsonParams(jal, "stuCode", MyTools.StrFiltr(jsonV.get(0)));
				response.getWriter().write(jal.toString());
			} catch (Exception e) {
				// TODO: handle exception
    ...
    * @param MkszBean
    */
	private void getFormData(HttpServletRequest request, MkszBean bean){
