...
public class Svl_Kcbkmd extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		KcbkmdBean bean = new KcbkmdBean(request);
...
//				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
				...
				//查询导出课程下拉框
				jsonV = bean.loadExportKcCombo();
				jal = JsonUtil.addJsonParams(jal, "exportKcData", ((JSONArray)jsonV.get(2)).toString());
		...
		if("initStuScoreInfo".equalsIgnoreCase(active)){
			try {
				...
				jal = JsonUtil.addJsonParams(jal, "llData", MyTools.StrFiltr(jsonV.get(0)));
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
    ...
    * @param MajorSetBean
    */
	private void getFormData(HttpServletRequest request, KcbkmdBean bean){
