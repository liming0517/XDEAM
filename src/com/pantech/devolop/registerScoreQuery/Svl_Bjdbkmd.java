...
public class Svl_Bjdbkmd extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		BjdbkmdBean bean = new BjdbkmdBean(request);
...
//				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
		...
		//查询学科
		if("loadic_njCombo".equalsIgnoreCase(active)){
			try {
				jsonV = bean.loadic_njCombo();
				jal = JsonUtil.addJsonParams(jal, "njData", ((JSONArray)jsonV.get(2)).toString());
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
				e.printStackTrace();
			}
		...
		if("initStuScoreInfo".equalsIgnoreCase(active)){
			...
			try {
				//查询学生成绩信息
				jsonV = bean.loadStuScoreInfo(pageNum, pageSize,XH,KCDM);
				jal = (JSONArray)jsonV.get(2);
				response.getWriter().write("{\"total\":" + MyTools.StrFiltr(jsonV.get(0)) + ",\"rows\":" + MyTools.StrFiltr(jal.toString()) + "}");
			} catch (SQLException e) {
				// TODO Auto-generated catch block
    ...
    * @param MajorSetBean
    */
	private void getFormData(HttpServletRequest request, BjdbkmdBean bean){
