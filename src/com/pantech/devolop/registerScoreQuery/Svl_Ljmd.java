...
public class Svl_Ljmd extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		LjmdBean bean = new LjmdBean(request);
...
//				jal = JsonUtil.addJsonParams(jal, "listData", "{\"total\":" + MyTools.StrFiltr(jsonV.get(0))+ ",\"rows\":" + jal.toString() + "}");
		...
		//获取初始化学生成绩信息
		if("initStuInfo".equalsIgnoreCase(active)){
			try {
				//查询学生成绩信息
				jsonV = bean.loadStuInfo();
				jal = JsonUtil.addJsonParams(jal, "colData", ((JSONArray)jsonV.get(2)).toString());
				response.getWriter().write(jal.toString());
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
    ...
    * @param MajorSetBean
    */
	private void getFormData(HttpServletRequest request, LjmdBean bean){
		...
		bean.setSFLJ(MyTools.StrFiltr(request.getParameter("SFLJ"))); //是否留级
	}
}