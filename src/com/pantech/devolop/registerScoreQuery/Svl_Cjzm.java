...
public class Svl_Cjzm extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		CjzmBean bean=new CjzmBean(request);//对象
		...
		if("exportScoreInfo2".equalsIgnoreCase(active)){
			...
			try {
				String filePath;
				try {
					filePath = bean.exportScoreInfo2(XBDM,NJDM,SSZY,BJBH,exportType,JX_XJH,XM);
					jal = JsonUtil.addJsonParams(jal, "MSG", bean.getMSG());
		...
		if("exportScoreInfo".equalsIgnoreCase(active)){
			...
			try {
				...
				if("studentInfo".equalsIgnoreCase(exportType)){
						linkStr = PageOfficeLink.openWindow(request, "form/registerScoreManage/CjzmView.jsp?exportType="+ exportType+
	...
	if("loadTeaPageOfficeLink".equalsIgnoreCase(active)){
		...
		try {
		String linkStr = "";
		if("xnxqXqk".equalsIgnoreCase(exportType)){
				linkStr = PageOfficeLink.openWindow(request, "form/registerScoreManage/CjzmView.jsp?exportType="+ exportType+
						"&JX_XJH="+JX_XJH+ "&BJBH=" + BJBH +"&XM=" + URLEncoder.encode(URLEncoder.encode(URLEncoder.encode(XM, "UTF-8"), "UTF-8"), "UTF-8")   + "","width=1000px;height=800px;");
    ...
    * @param XsxjkBean
    */
	private void getFormData(HttpServletRequest request, CjzmBean bean) {
