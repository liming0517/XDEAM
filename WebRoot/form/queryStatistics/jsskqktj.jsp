...
<script type="text/javascript">
	...
	function loadGrid(){ 
		$('#List').datagrid({
				...
				queryParams:{"active":"teaskqk","XNXQBM":$('#XNXQ').combobox('getValue'),"TEAID":$('#teaId').val(),"TEANAME":$('#teaName').val()},
				loadMsg : "信息加载中请稍侯!",
				fit:true,
				width:'100%',
				nowrap:false,
				...
				fitColumns:true,
				columns:[[
					{field:'学年学期编码',title:'学年学期',width:fillsize(0.22),align:'left',
					...
					{field:'授课教师编号',title:'教师工号',width:fillsize(0.2),align:'left'},
					{field:'授课教师姓名',title:'教师姓名',width:fillsize(0.15),align:'left'},
// 					{field:'层级名称',title:'教师类别',width:fillsize(0.2),align:'left'},
					{field:'专业名称',title:'所属专业',width:fillsize(0.2),align:'left'},
					{field:'课程代码',title:'课程代码',width:fillsize(0.2),align:'left'},
					{field:'课程名称',title:'课程名称',width:fillsize(0.2),align:'left'},
					{field:'考试形式',title:'考试形式',width:fillsize(0.2),align:'left'},
					{field:'行政班名称',title:'班级名称',width:fillsize(0.25),align:'left'},
					{field:'总人数',title:'班级人数',width:fillsize(0.15),align:'left'},
					{field:'时间序列',title:'星期',width:fillsize(0.15),align:'left',
						formatter:function(value,rec){
							var sjxl="";
							if(rec.时间序列=="01"){
								sjxl="星期一";
					...
					{field:'总课时',title:'课程课时',width:fillsize(0.15),align:'left',
						formatter:function(value,rec){ 
							var zs=0;
					...
					{field:'授课周数',title:'周数',width:fillsize(0.1),align:'left',
						formatter:function(value,rec){ 
							var zs=0;
							zs=rec.授课周数-rec.假日周次;
							return zs;
					...
					{field:'场地名称',title:'教室',width:fillsize(0.12),align:'left'}
				]],
		...
		function doToolbar(iToolbar){
			...
			if(iToolbar=="export"){					
				...
				if(pofOpenType == 'normal'){ 
					...
					$("#exportTimetableiframe").attr("src","<%=request.getContextPath()%>/form/timetableQuery/exportWorkLoad.jsp?exportType=teachSituation&XNXQ=" + encodeURI(encodeURI(tempxnxq)) + "&TEAID=" + encodeURI(encodeURI(tempjsbh)) + "&TEANAME=" + encodeURI(encodeURI(tempjsxm)) );
				}else{ 
					$.ajax({
						type : "POST",
						url : '<%=request.getContextPath()%>/Svl_Kbcx',
						data : "active=loadWorkPageOfficeLink&exportType=teachSituation&XNXQ=" + encodeURI(encodeURI(tempxnxq)) + "&TEAID=" + encodeURI(encodeURI(tempjsbh)) + "&TEANAME=" + encodeURI(encodeURI(tempjsxm)) ,
						dataType:"json",
