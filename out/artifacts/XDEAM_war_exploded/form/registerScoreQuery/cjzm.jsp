...
<script type="text/javascript">
	...
	function initExportData(){
		$.ajax({
			type : "POST",
			url : '<%=request.getContextPath()%>/Svl_Cjzm',
	...
	**/
	...
	function loadBJCombobox(){
		$('#ic_bjmc').combobox({
			url:'<%=request.getContextPath()%>/Svl_Cjzm?active=loadExportBjCombo&NJDM='+$('#ic_njdm').combobox('getValues') + 
		...
		$('#XSXXList').datagrid({
			...
			columns:[[
				...
				{field:'AA_SET',align:'center',title:'操作',width:fillsize(0.13),formatter:function(value,rec){
					return "<input type='button' value='[成绩信息]' onclick='stuInfo(\"" + rec.学号 +"\",\"" + rec.姓名 +"\",\"" + rec.行政班名称 + "\",\"" + rec.专业名称 +"\",\"" + rec.学生状态 +"\",\"" + rec.行政班代码 +"\")' id='stuInfo' style='margin-left:5px; cursor:pointer;'>";
			  }}
			]],
			//当用户点击某行时触发
	...
	function stuInfo(stuCode, stuName, BJMC, YZYMC, stuState, bjdm){
		if(pofOpenType == 'normal'){
			...
			$("#printViewIframe").attr("src","<%=request.getContextPath()%>/form/registerScoreManage/stuInfoView.jsp?exportType=xnxqXqk&JX_XJH="+stuCode+"&BJDM="+bjdm+"&XSLX=All");
		}else{
			$.ajax({
				type : "POST",
				url : '<%=request.getContextPath()%>/Svl_Cjzm',
				data : "active=loadTeaPageOfficeLink&exportType=xnxqXqk&JX_XJH="+stuCode+"&BJBH="+bjdm+"&XM="+stuName,
				dataType:"json",
				success : function(data){
